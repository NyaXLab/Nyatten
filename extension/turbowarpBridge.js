(function() {
		...
		if (msg && msg.type === 'nyatten:fetch-asset-response') {
			...
			if (pending) {
				...
				if (success) {
					...
					pending.reject(new TypeError('Failed to fetch (Nyatten Bridge)'));
				}
			}
		}
	});
	...
	const observer = new MutationObserver(() => {
		...
		const checkbox = document.querySelector('input.load-extension_unsandboxed-checkbox_1tSmf[type="checkbox"]');
		...
		// Target 'Allow' button
		const allowBtn = document.querySelector('button.security-manager-modal_allow-button_3tcXk');
		// Only click if the button exists and is not disabled
		if (allowBtn && !allowBtn.disabled && !allowBtn.hasAttribute('disabled')) {
			allowBtn.click();
